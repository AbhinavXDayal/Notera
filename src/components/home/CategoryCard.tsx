...
export const CategoryCard: React.FC<CategoryCardProps> = ({
  ...
  return (
    <div
      ...
      className={`border rounded-xl p-5 flex flex-col justify-between transition-all duration-200 relative shadow-terra-card ${
        isRecommended && isAvailable
          ? "cursor-pointer bg-surface border-primary ring-1 ring-primary/30 hover:shadow-terra-hover transform hover:-translate-y-0.5 group"
          : isAvailable
    ...
    >
      {/* Recommended or Unavailable Badge */}
      {isRecommended && isAvailable && (
        <div className="absolute top-3.5 right-3.5">
          <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded text-[9px] font-mono tracking-wider bg-primary/10 border border-primary/30 text-primary uppercase font-semibold">
